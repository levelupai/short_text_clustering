...
# in this program we will classify then cluster the text of the ubuntudataset with labels windows  and non-windows
...
cluster_0= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster0.csv', 'w+')
cluster_1= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster1.csv', 'w+')
cluster_2= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster2.txt', 'w+')
cluster_0_ms= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster0_ms.csv', 'w+')
cluster_1_ms= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster1_ms.csv', 'w+')
cluster_2_ms= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster2_ms.csv', 'w+')
cluster_3_ms= open('/home/levelup/Ayushi_folder/windows/ubuntu_complete_sentences_10k_output_cluster3_ms.csv', 'w+')
vectors_d= open('/home/levelup/Ayushi_folder/windows/vectors.csv', 'w+')
# pre processing the data
# pre processing the data
# the data contains two columns one is the content column and the other is the labe column which shows where the contenet of the chat contains the converation about the
# windows topic or not
...
model = Doc2Vec(vector_size=2, window=15,min_count=5,workers=4, alpha=0.1,
min_alpha=0.001)
doc2vec_model_X = model.build_vocab(sentenceLabeled)
cc=model.docvecs.vectors_docs
...
for i in range(len(cc)):
    vectors_d.write("%s,%s\n" % (cc[i][0], cc[i][1]))
    vectors_d.flush()
    x_coords.append(cc[i][0])
    y_coords.append(cc[i][1])
    if line_labels_in_order[i] == -1:
        color = "orange"
    else:
        color = "blue"
    colors.append(color)
...
datasets = [
    (sentenceLabeled, {'damping': .77, 'preference': -240, 'quantile': .2, 'n_clusters': 2})]
...
for i_dataset, (dataset, algo_params) in enumerate(datasets):
    ...
    for name, algorithm in clustering_algorithms:
        ...
        with warnings.catch_warnings():
            ...
            wordfreq= []
            total_lines=[]
            # printing the clustering results for only two algorithms, similarly print for more if needed
...
n_samples = len(sentenceLabeled)
n_clusters_range = np.linspace(2, 20000, 10).astype(np.int)
n_classes = 2
...
n_samples = len(sentenceLabeled)
l= np.array(model.docvecs.vectors_docs)
X = l
y = line_labels_in_order
